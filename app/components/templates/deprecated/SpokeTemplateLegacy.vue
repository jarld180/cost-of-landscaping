...
const showSidebar = computed(() => {
  return metadata.value?.showSidebar !== false && navigationSections.value.length > 0
})

// Determine sidebar position
const sidebarPosition = computed(() => {
  return metadata.value?.sidebarPosition || 'left'
})
...
<template>
  <div class="min-h-screen bg-neutral-50 dark:bg-neutral-900">
    <div class="mx-auto max-w-8xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div
        class="grid grid-cols-1 gap-8 lg:gap-12"
        :class="showSidebar ? 'lg:grid-cols-4' : 'lg:grid-cols-1'"
      >
        <!-- Sidebar Navigation (Left Position) -->
        <div
        ...
        <!-- Main Content Area -->
        <article :class="showSidebar ? 'lg:col-span-3' : 'lg:col-span-1 mx-auto max-w-4xl'">
          <!-- Hero Section -->
          <section class="mb-12">
            <!-- Eyebrow Badge -->
            ...
            <p
              ...
              class="mb-6 text-lg leading-relaxed text-neutral-600 dark:text-neutral-300 md:text-xl"
            >
              {{ page.description }}
            </p>
        ...
        <!-- Sidebar Navigation (Right Position) -->
        <div
